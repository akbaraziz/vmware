...
password = "@@{vc.secret}@@"
# retreived from VcSoapGetObjects
vm_folder_root_id = "@@{vc_vm_folder_root_id}@@"
...
headers = {'Content-Type': 'application/xml',
           'Accept': 'application/xml', 'Cookie': vc_cookie}
# endregion
    ...
    for element in resp_parse.iter('*'):
        ...
        if "faultstring" in element.tag:
            if "already exists" in element.text:
                ...
                payload_find = payload_parse.find(
                    ".//{urn:vim25}FindByInventoryPath")
                payload_push = ET.SubElement(payload_find, "inventoryPath")
                payload_push.text = "/{0}/vm/{1}".format(
                    datacenter, vm_folder_name)
